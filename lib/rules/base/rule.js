...
Rule.prototype = {
	...
	error: function (index) {
		return new Rule.ValidationError(this.name, this.serialize(), index);
	},
	...
	convertNestedError: function (err, index) {
		if (!(err instanceof Rule.ValidationError)) {
			return new Error('#R1: argument error must be instance of Rule.ValidationError in rule "' + this.name + '"');
		}
